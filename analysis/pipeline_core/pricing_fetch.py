"""SMARD price fetching utilities."""
...
def fetch_smard_prices(
    ...
    *,
    ...
    try:
        ...
        timestamps = sorted((int(ts) // 1000) for ts in index_response.json().get("timestamps", []))
    ...
    if not block_list:
        logger.warning("No SMARD data blocks covering %s - %s for region=%s.", min_ts, max_ts, region)
        return None
    ...
    logger.info("Fetched %s SMARD price points for %s-%s (%s).", price_df.height, min_ts, max_ts, region)
    return price_df
